        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              >
                ...
                <dd
                  ...
                  v-for="spuSaleAttrValue in saleAttrList.spuSaleAttrValueList"
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 之前的路由跳转：从A路由跳转到B路由，这里的购物车，进行路由跳转之前，发请求
...
export default {
  ...
  mounted() {
    // 派发actions 获取商品详情页的信息
    this.$store.dispatch('detailAbout/getGoodsInfo', this.$route.params.skuid)
  },
  methods: {
    // 点击产品售卖属性的属性值切换高亮
    changeActive(spuSaleAttrValue, arr) {
      // console.log(spuSaleAttrValue)
      // 循环遍历 将所有的 isChecked 变为 0 ， 将点击的那个isChecked 变为 1
      arr.forEach((item) => {
        item.isChecked = 0
      })
      // 点击的那个售卖属性值为1
    ...
    async addShopCar() {
      ...
      try {
        ...
        // 本地存储| 会话存储 ，一般存储的是字符串 这里使用JSON.stringify() 转化为字符串
